...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['capnhatthongtin'])) {
        ...
        WHERE 
         ...
         if($conn->query($sql1) == TRUE){
            echo '<div class="list">Cập nhật thông tin cá nhân thành công. <a href="indexTaikhoan.php?&id='.$id.'">Quay lại</a></div>';
            exit;
        } else {
            echo '<div class="list">Đã có lỗi xảy ra. <a href="indexTaikhoan.php?&id='.$id.'">Quay lại</a></div>';
            exit;
    }
    }

    echo '<div class="vien"><div class="center">';
    echo '<form action="indexTaikhoan.php?mod=capnhat&id='.$id.'" method="POST">';
    ...
    echo'<p><input type="submit" name="capnhatthongtin" value="Cập nhật" /></p>';
    echo'</form>';
    echo'</div></div>';
