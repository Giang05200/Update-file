...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['capnhatthongtin'])) {
        ...
        if (mysqli_query($conn, $sql)) {
            echo '<div class="list">Cập nhật thông tin cá nhân thành công. <a href="indexQLTK.php?&id='.$id.'">Quay lại</a></div>';
            exit;
        } else {
            echo '<div class="list">Đã có lỗi xảy ra. <a href="indexQLTK.php?&id='.$id.'">Quay lại</a></div>';
            exit;
    }
    }

    echo '<div class="vien"><div class="center">';
    echo '<form action="indexQLTK.php?mod=sua&id='.$id.'" method="POST">';
