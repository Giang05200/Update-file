...
<?php
 if ($ok != 2)
  {
  echo '<div class="list">Không có sản phẩm nào. <a href="trangchu.php">Quay lại cửa hàng</a> để tiếp tục mua sắm.</div></div>';
  ...
  echo '<table width="100%" style="border:1px solid #ebebeb; color: white; border-collapse:collapse;">';
  echo'<tr>
  ...
  while ($row = mysqli_fetch_array($query)) {
    ...
    echo'<tr><td width="20%" style="border:1px solid #ebebeb; text-align: center;"><img src="image/'.$row['HinhURL'].'" width="100" height="100"></td>';
    echo'<td width="40%" style="border:1px solid #ebebeb; color: Black;">'.$row['TenSanPham'].'</td>';
    echo'<td width="10%" style="border:1px solid #ebebeb; text-align: center;"><font color="##008744"><b>';
    echo number_format($row['GiaSanPham'], 0).' đ</br><del class="product-old-price" style="color: black;">'.number_format($row['GiaHang'], 0).'đ</del></font></td>';
    echo '<td width="10%" style="border:1px solid #ebebeb; color: Black; text-align: center;">';
    echo'<form action="indexGiohang.php?mod=update&id='.$row['MaSanPham'].'" method="POST">';
?>
    ...
    color: #00ff00;"> <?php echo $soluong; ?></p>
    ...
    echo '<td width="5%"style="border:1px solid #ebebeb; text-align: center;"><a href="indexGiohang.php?mod=xoa&id='.$row['MaSanPham'].'"><i class="far fa-trash-alt" aria-hidden="true" style="
    font-size: 25px;
    text-decoration: none;
    color: red;
">Xóa</i></a></td>';
  ...
  echo '<form action="indexGiohang.php?mod=thanhtoan" method="POST">';
