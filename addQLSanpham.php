...
if (isset($_POST['themsp'])) {
    ...
    if(isset($_POST["GiaHang"])) { $GiaHang = $_POST['GiaHang']; }
...
if (isset($_POST['themsp'])) {
    ...
    if (isset($_FILES['Hinhsp']) && $_FILES['Hinhsp']['error'] == 0) {
        ...
        if(file_exists("image/" . $_FILES["Hinhsp"]["name"])){
        echo $_FILES["Hinhsp"]["name"] . " đã tồn tại";
        exit;
        } else{
        // Hàm move_uploaded_file sẽ tiến hành upload file lên thư mục upload
        move_uploaded_file($_FILES["Hinhsp"]["tmp_name"], "image/" . $_FILES["Hinhsp"]["name"]);
    ...
    if (mysqli_query($conn, $sql)) {
        echo "<div class='list'>Thêm sản phẩm thành công. <a href='indexQLSanpham.php'>Quay lại</a></div>";
        exit;
    } else {
...
<div id="vien">
    <div class="center">
        <div id="ban">
            <a id="ba" href="trangchu.php">Trang chủ</a> > 
            <a id="ba" href="indexAdmin">Admin Panel</a> > 
            <a id="ba" href="indexQLSanpham.php?mod=panel">Quản lý sản phẩm</a> > 
            <font color="#008744">Thêm sản phẩm</font>
        </div>
    </div>
</div>
<div class="list">
    <form action='indexQLSanpham.php?mod=add' method='POST' enctype="multipart/form-data">
        <table>
            <tr>
                <td>
                    <p>Tên sản phẩm </p>
                    <p><input id="TenSanPham" type='text' size="20" name='TenSanPham' /></p>
                </td><td>
                    <p>Giá sản phẩm </p>
            ...
            <td>
                    <p>Giá hàng </p>
                    <p><input id="GiaHang" type='number' size="50" name='GiaHang' /></p>
                </td></tr>
                <tr><td>
                    <p>Ngày nhập hàng </p>
        ...
        <p><select name="HangSX">
                        ...
                        $thuonghieu = mysqli_query($conn, $th);
        ...
        <a href='indexQLSanpham.php?mod=panel'>Quay lại</a></p>
    </form>
</div>