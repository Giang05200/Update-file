...
<div class="list"><a href="indexQLSanpham.php?mod=add"><button class="button">Thêm sản phẩm</button></a></div>
<?php
// 2. Tạo câu truy vấn (Query): SELECT, INSERT, DELETE, UPDATE
    ...
    while($row = mysqli_fetch_array($result))
    {
        ...
        echo '<br/><img width="100" height="100" src="image/'.$row['HinhURL'].'"><br/>';
        echo '<div class="tool"><a style="color: #00ff00;" href="indexQLSanpham.php?mod=update&id='.$row['MaSanPham'].'"><i class="far fa-edit">Sửa</i></a></br>  ';
        if($row['BiXoa'] == 1) {
            echo '<a href="indexQLSanpham.php?mod=restore&id='.$row['MaSanPham'].'"><i class="fas fa-trash-restore-alt"></i></a>';
        } else {
            echo '<a style="color: red;" href="indexQLSanpham.php?mod=del&id='.$row['MaSanPham'].'"><i class="far fa-trash-alt">Xóa</i></a>';
        }
        echo '</div></div>';
    }
?>
